 ...
 */
function parseScheduleData(): Game[] {
  const games: Game[] = [];
  
  // Check if there's uploaded data in localStorage
  const uploadedData = getScheduleFromStorage();
...
/**
 * Get the current schedule games (checks localStorage for updates)
 */
export function getScheduleGames(): Game[] {
  return parseScheduleData();
}

// Export a static version for initial load
export const scheduleGames = parseScheduleData();
